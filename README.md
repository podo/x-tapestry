...
feed with either comma-separated handles or a raw X search query. It renders
media, quoted posts, polls, profile photos, and X-style URL preview cards. When
X only returns a plain expanded URL, the connector can fetch cached webpage
metadata to build a richer card. Feed Finder can also prefill a handle from an
`@handle`, X profile URL, Twitter profile URL, or status URL.
