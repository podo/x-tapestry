...
- Card chrome order (Loom `post` style): native annotations for **Reposted by @…** / **Reply** (when present) then **`X · {Feed Type}`** above Author. Hide static Service chrome (`default_service_name_visibility: "hidden"`) so bare `service_name` "X" does not duplicate the feed-type annotation. Then Author → body meta (metrics in `<small>`) → caption → attachments → actions. Retweet Identity is the original author. Do not emit feed title or article host under Author.
- Verify/feed list titles use the same `X · …` string via `feedDisplayName()`.
