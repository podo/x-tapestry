...
- Card chrome order (Loom `post` style): native annotations only for **Reposted by @…** and **Reply** (above Service); then Service → Author → body meta (metrics in `<small>`) → caption → attachments → actions. Retweet Identity is the original author. Do not emit a duplicate article-host line under Author; caption links + LinkAttachment already cover that.
- Keep `default_service_name_visibility: "visible"` so the card Service line shows connector `service_name` ("X"). Feed titles from verify (`X · Following Feed`, etc.) name the feed in the feed list only. Per-feed Service text on the card is not available in the Tapestry connector API today (needs an Iconfactory change); do not fake it with a second body `X · …` line under Author.
