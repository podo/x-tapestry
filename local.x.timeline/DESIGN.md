...
- Card chrome order (Loom `post` style): native annotations for **Reposted by @…** / **Reply** (when present) then **`X · {Feed Type}`** (above Service); then Service (`service_name` "X", visibility visible) → Author → body meta (metrics in `<small>`) → caption → attachments → actions. Retweet Identity is the original author. Do not emit a duplicate article-host or feed-title line under Author—Service text cannot be per-feed in the connector API, so feed type is an annotation above Service instead.
- Keep `default_service_name_visibility: "visible"`. Verify/feed list titles use the same `X · …` string via `feedDisplayName()`.
