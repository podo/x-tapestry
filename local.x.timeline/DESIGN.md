...
- Card chrome order (Loom `post` style): native annotations for **Reposted by @…** / **Reply** only (above Service). Service chrome uses `service_name` (`X · Following Feed`) at native Service size — connectors cannot set this per `source_mode`. Then Author → body meta (metrics in `<small>`) → caption → attachments → actions. Do not also emit feed type as an annotation (that renders at Reply-to size).
- Verify/feed list titles still use dynamic `feedDisplayName()` (`X · …`).
